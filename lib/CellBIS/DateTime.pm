...
sub list_date {
  ...
  if (ref($start) eq "ARRAY") {
    $start_new = DateTime->new(@{$start});
  } else {
    $start_new = DateTime->now();
  }

  if (ref($end) eq "ARRAY") {
    $end_new = DateTime->new(@{$end});
  } else {
...
#    print "dsc\n";
    while ($start_new > $end_new) {
      push @data, $start_new->ymd('-');
      $start_new->subtract( days => 1 );
    }
    push @data, $start_new->ymd('-');
    $start_new->subtract( days => 1 );
  }
...
sub get {
  # Define parameter subroutine :
  my $self = shift;
  my ($timestamp, $timezone, $attribute) = @_;
...
#  my $epoch_time = $dt->epoch();
  ...
  if (ref($attribute) eq 'HASH') {
    ...
    # For Config :
    my $delim_date = exists $attribute->{'date'} ? $attribute->{'date'} : '-';
    my $delim_time = exists $attribute->{'time'} ? $attribute->{'time'} : ':';
    my $delim_datetime = exists $attribute->{'datetime'} ? $attribute->{'datetime'} : ' ';
    my $minplus_datetime = exists $attribute->{'minplus'} ? $attribute->{'minplus'} : '';
...
#    print "MIN-PLUS $minplus_datetime\n";
    ...
    $format_datetime =~ s/Mn/$nama_bulan/g;
...
# Get data time from date :
# ------------------------------------------------------------------------
sub get_from_date {
  my $self = shift;
  my ($timezone, $attribute, $year, $month, $day) = @_;
  my %data = ();
...
NKTIweb::DateTime - Module for DateTime handling.
...
=head1 SYNOPSIS
  ...
  # Initialize :
  my $cb_dt = CellBIS::DateTime->new();



=head1 DESCRIPTION

This module based on module L<DateTime> in CPAN.
