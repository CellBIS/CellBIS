...
#	@arr_string = grep { $_ } @arr_string;
	...
	my $i = 0;
	my $key_arrstr = scalar @arr_string;
	...
	while ($i < $until_loop) {
		...
		if (($i % 2) == 1) {
			...
			# Placing Even Index into scalar $data :
			$data_err .= $arr_string[$i];
		}
		# End of check IF $i % 2 eq 1.

#		print "Hasil String odd_index = $data\n";
	...
	my $leng_data = length $data;
	print "Hasil nkti_odd_index = $data [$leng_data]\n";
...
#	@arr_string = grep { $_ } @arr_string;
	...
	my $i = 0;
#	my $key_arrstr = scalar @arr_string;
	my $until_loop = scalar @arr_string;
	...
	while ($i < $until_loop) {
		...
		# End of check IF $i % 2 == 0.
		# =================================================================

#		print "Hasil String even_index = $data\n";
	...
	my $leng_data = length $data;
	print "Hasil nkti_even_index = $data [$leng_data]\n";
...
sub loop_odoe2eood {
	...
	my $count_even2odd = 0;
	my $status_nested = 0;
	my $sizeof_args = scalar @_;

	print Dumper \@_;
	print "loop_odoe2eood - action \n";
	...
	if ($status_nested == 1) {
		...
		# For ODD 2 Even :
		if ($count_odd2even != 0 and $count_even2odd != 0) {
		...
		# For EVEN 2 Odd :
		elsif ($count_odd2even != 0 and $count_even2odd == 0) {
...
sub extract_loop_odoe2eood {
	...
	print "extract_loop_odoe2eood - action \n";
	print "data to extract = $string [$len_string]\n";
	...
	# Check IF $status_nested == 2 :
		...
		# For EVEN 2 Odd Extract :
		if ($count_odd2even != 0 and $count_odd2even == 0) {
	...
	# Check IF $status_nested == 0 :
		...
		if ($count_odd2even != 0 and $count_even2odd != 0) {
			...
			my $leng_r1 = length $result1;
			my $leng_r = length $result;
			print "\$result1 = $result1 [$leng_r1]\n";
			print "\$result = $result [$leng_r]\n";
		}
