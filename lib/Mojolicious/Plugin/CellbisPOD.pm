...
sub register {
  ...
  $app->helper(
    pod_to_html => sub {
      shift;
      Mojo::ByteStream->new(_pod_to_html(@_))
    });
  ...
  if (exists $conf->{default}) {
    $defaults = { module => $conf->{default}, format => 'html' };
  }
  else {
    $defaults = { module => 'Mojolicious/Guides', format => 'html' };
  }
  return $app->routes->any(
    '/cbtm/:module' => $defaults => [ module => qr/[^.]+/ ] => \&_perldoc);
}
...
sub _html {
  ...
  $c->render('cb_perldoc', title => $title, parts => \@parts);
}
...
sub _perldoc {
  ...
  my $path;
  $path = Pod::Simple::Search->new->find($module, map {$_, "$_/pods"} @INC);
  #	unless ($path && -r $path) {
  #		my $default_path = blessed($c->app) if blessed($c->app);
  #		$path = Pod::Simple::Search->new->find($default_path, @INC);
  #	}
  return $c->redirect_to('/cbtm') unless ($path && -r $path);
  
  #	say "Path : $path";
  my $src = path($path)->slurp;
  #	$c->respond_to(txt => {data => $src}, html => sub { _html($c, $src) });
  $c->respond_to(html => sub {_html($c, $src)});
}
...
Mojolicious::Plugin::CellbisPOD - POD renderer plugin
...
L<Mojolicious::Plugin::CellbisPOD> is a plugin based on L<Mojolicious::Plugin::PODRenderer>
...
=head2 default
  ...
  # Mojolicious::Lite
  plugin PODRenderer => {default => 'ModuleName'};

Name of handler used to certain default POD to view.
